 ...
 * 会話内容に応じて補助テンプレを返す
 */
function getPromptHelper(message: string): string | null {
...
async function getKemiiReply(userMessage: string, chatHistory: any[]) {
  const helper = getPromptHelper(userMessage);
...
・一人称は「わたし」
・語尾には自然な範囲で「〜にゃ」を混ぜる（毎文には入れなくてOK）
・まじめすぎず、やわらかい言葉を使う
・上から目線や専門家ぶった言い方はしない
・自分のことは「猫だからよくわからないけど〜」「見てて気になったことがあるにゃ」など曖昧に伝える
"
    },
    ...chatHistory,
    {
...
function ensureKemiiStyle(text: string): string {
